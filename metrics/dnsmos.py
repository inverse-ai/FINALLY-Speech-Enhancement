...
class DNSMOS_local:
    ...
    def __init__(
        self, primary_model_path=None, p808_model_path=None, use_gpu=False, convert_to_torch=False, device=None
    ):
        ...
        if convert_to_torch:
            ...
            if use_gpu:
                if primary_model_path is not None:
                    ...
                    self.p808_model = self.p808_model.to(device)
    ...
    def __call__(self, fpath=None, aud=None, input_fs=None, is_personalized_MOS=False):
        if fpath is not None:
            aud, input_fs = librosa.load(fpath, sr=None, mono=True, dtype=np.float32)
        if self.convert_to_torch:
            if isinstance(aud, torch.Tensor):
                aud = aud.to(device=self.device)
            else:
                aud = torch.as_tensor(aud, dtype=torch.float32, device=self.device)
        else:
        ...
        if self.convert_to_torch:
            if audio.size(0) < len_samples:
                repeat_factor = -(-len_samples // audio.size(0))  # Ceiling division
                audio = audio.repeat(repeat_factor)
            # No truncation
        else:
            if len(audio) < len_samples:
                repeat_factor = -(-len_samples // len(audio))  # Ceiling division
                audio = np.tile(audio, repeat_factor)
            # No truncation

        # (audio_len - inp_len + 1) / 8
        num_hops = int((np.floor(len(audio) / SAMPLING_RATE) - INPUT_LENGTH + 1) // 8)

        hop_len_samples = SAMPLING_RATE * 8  # 8 seconds hop length
        ...
        for idx in range(num_hops):
            audio_seg = audio[
                int(idx * hop_len_samples) : int((idx * hop_len_samples) + len_samples)
            ]
        ...
        # create a window from last and pass into model
        if len(audio) > len_samples:
        ...
        if self.use_gpu:
            return {
                # "filename": fpath.split('/')[-1] if fpath else "audio",
                "OVRL": to_array(predicted_mos_ovr_seg).mean().cpu().item(),
                # "SIG": to_array(predicted_mos_sig_seg).mean().cpu().item(),
                # "BAK": to_array(predicted_mos_bak_seg).mean().cpu().item(),
                "P808": to_array(predicted_p808_mos).mean().cpu().item(),
            }
        return {
            "filename": fpath.split('/')[-1] if fpath else "audio",
            "OVRL": to_array(predicted_mos_ovr_seg).mean().item(),
            "SIG": to_array(predicted_mos_sig_seg).mean().item(),
            "BAK": to_array(predicted_mos_bak_seg).mean().item(),
            "P808": to_array(predicted_p808_mos).mean().item(),
        }
