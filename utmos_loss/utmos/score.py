...
class ChangeSampleRate(nn.Module):
    ...
    def forward(self, wav: torch.tensor) -> torch.tensor:
        ...
        indices = (torch.arange(new_length).to(wav.device) * (self.input_rate / self.output_rate))
...
class Score:
    def __init__(self, device='cpu'):
        self.model = BaselineLightningModule.load_from_checkpoint(cached_path('hf://mosmodels/utmos/model.ckpt'), device).eval().to(device)
