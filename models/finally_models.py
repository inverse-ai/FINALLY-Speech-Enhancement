...
class FinallyGenerator(A2AHiFiPlusGeneratorV2):
    ...
    def forward(self, x, wavlm_features):
        ...
        # print("Shape of x after spectralunet:", x.shape)
        # print("Min of x after spectralunet:", x.min().item())
        # print("Max of x after spectralunet:", x.max().item())
        # print("Mean of x after spectralunet:", x.mean().item())
        # print("Std of x after spectralunet:", x.std().item())
        # print("Number of zeros in x after spectralunet:", (x == 0).sum().item())
        # print("Number of NaNs in x after spectralunet:", torch.isnan(x).sum().item())
        # print("Number of Infs in x after spectralunet:", torch.isinf(x).sum().item())
        # print("Percentage of NaNs in x after spectralunet", torch.isnan(x).float().mean().item() * 100)
        ...
        if self.use_tanh:                 
            x = torch.tanh(x)
            # Normalize the output to be in [-0.4  0.4]
            # max_val = x.abs().max()
            # if max_val > 0:
            #     x = x / max_val * 0.4
