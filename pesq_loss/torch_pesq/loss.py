...
class PesqLoss(torch.nn.Module):
    ...
    def __init__(
        ...
        self.fbank = BarkScale(device, n_fft // 2, nbarks)

        # set up loudness degation and calibration
        self.loudness = Loudness(device, nbarks)
    ...
    def raw(
        ...
        deviceDeg = deg.device
        deviceRef = ref.device
        deg, ref = self.resampler(deg.cpu()).to(deviceDeg), self.resampler(ref.cpu()).to(deviceRef)
        ...
        deg, ref = self.to_spec(deg.cpu()).swapaxes(1, 2).to(deviceDeg), self.to_spec(ref.cpu()).swapaxes(1, 2).to(deviceRef)
