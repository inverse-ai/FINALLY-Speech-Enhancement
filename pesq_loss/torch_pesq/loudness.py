...
class Loudness(torch.nn.Module):
    ...
    def __init__(self, device: str = 'cpu', nbark: int = 49):
        super(Loudness, self).__init__()

        self.threshs = Parameter(
            interp(abs_thresh_power_16k, nbark).unsqueeze(0).unsqueeze(0).to(device),
            requires_grad=False,
        )

        exp = 6 / (torch.tensor(centre_of_band_bark_16k) + 2.0)
        self.exp = Parameter(
            exp.clamp(min=1.0, max=2.0).to(device) ** 0.15 * zwicker_power, requires_grad=False
        )
